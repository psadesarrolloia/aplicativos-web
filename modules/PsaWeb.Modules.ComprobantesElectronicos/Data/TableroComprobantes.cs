...
namespace PsaWeb.Modules.ComprobantesElectronicos.Data;
...
{
    /// <summary>Código SRI de la retención: vive en <c>TaxWithHoldings</c>, no en <c>Facturas</c>.</summary>
    private const string CodDocRetencion = "07";

    /// <summary>
    /// Retenciones guardadas (<c>TaxWithHoldings</c>) de una o varias empresas, mapeadas a la
    /// misma fila de listado que los otros tipos: el «Total» es el total retenido y no hay IVA.
    /// </summary>
    private async Task<IReadOnlyList<ComprobanteReciente>> RecientesRetencionesAsync(
        IReadOnlyList<string> rucs, DateTime? desde, DateTime? hasta, int top, CancellationToken cancellationToken)
    {
        ...
        if (desde is { } d) query = query.Where(t => t.DateIssued >= d.Date);
        if (hasta is { } h)
        {
        ...
        var ids = filas.Select(f => f.Thid).ToArray();
        var totales = await db.Thdetails.AsNoTracking()
            .Where(x => EF.Constant(ids).Contains(x.TaxWithHolding))
            .GroupBy(x => x.TaxWithHolding)
            .Select(g => new { Id = g.Key, Total = g.Sum(x => x.RtaxValue) })
            .ToListAsync(cancellationToken);
        var mapaTotal = totales.ToDictionary(x => x.Id, x => x.Total);
        ...
        var contactos = filas.Select(f => f.Contact).Where(c => !string.IsNullOrWhiteSpace(c)).Distinct().ToArray();
        var mapaPersona = new Dictionary<(string, string), string>();
        if (contactos.Length > 0)
        {
            var personas = await db.Persons.AsNoTracking()
                .Where(p => EF.Constant(rucArr).Contains(p.Ructransmitter) && EF.Constant(contactos).Contains(p.PersonId))
                .Select(p => new { p.Ructransmitter, p.PersonId, p.Name })
                .ToListAsync(cancellationToken);
            foreach (var p in personas) mapaPersona[(p.Ructransmitter, p.PersonId)] = p.Name;
        }

        return filas.Select(f => new ComprobanteReciente(
                f.Thid, f.TransmitterRuc, mapaEmpresa.GetValueOrDefault(f.TransmitterRuc, f.TransmitterRuc),
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (codDoc == CodDocRetencion) return await RecientesRetencionesAsync(rucs, desde, hasta, top, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var agregados = codDoc == CodDocRetencion
    ...
    {
        // Los pendientes de retención ya excluyen las emitidas (PendientesRepository): nada que descontar.
        if (rucs.Count == 0 || codDoc == CodDocRetencion) return new HashSet<string>(StringComparer.Ordinal);
        await using var db = await contextFactory.CreateDbContextAsync(cancellationToken);
        var rucArr = rucs.Distinct().ToArray();
    ...
    {
        if (codDoc == CodDocRetencion) return new HashSet<string>(StringComparer.Ordinal);
        await using var db = await contextFactory.CreateDbContextAsync(cancellationToken);
